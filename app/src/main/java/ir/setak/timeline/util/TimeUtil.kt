...
object TimeUtil {
    ...
    fun format(time: Long) = when (time) {
        in 0 until A_SECOND -> (time / A_SECOND).toString() + " seconds ago"
        in A_MINUTE until 2 * A_MINUTE -> "1 minute ago"
        in 2 * A_MINUTE until AN_HOUR -> (time / A_MINUTE).toString() + " minutes ago"
        in AN_HOUR until 2 * AN_HOUR -> "1 hour ago"
        in 2 * AN_HOUR until A_DAY -> (time / AN_HOUR).toString() + " hours ago"
        in A_DAY until 2 * A_DAY -> "1 day ago"
        in 2 * A_DAY until 7 * A_DAY -> (time / A_DAY).toString() + " day ago"
